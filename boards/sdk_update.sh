...
OPENWIFI_HW_IMG_DIR="zed_fmcs2"
echo $BOARD_NAME
echo $OPENWIFI_HW_IMG_DIR

TARGET_SDK_DIR=$BOARD_NAME/sdk/
...
IMPL_DIR="$BOARD_NAME/openwifi_$BOARD_NAME/openwifi_$BOARD_NAME.runs/impl_1"

cp $IMPL_DIR/system_top.* $TARGET_SDK_DIR/ -rf
if [ -f "$IMPL_DIR/system_top.ltx" ]; then
    cp $IMPL_DIR/system_top.ltx $TARGET_SDK_DIR/ -rf
else
